...
class MySQLHandler(object):
    ...
    def __init__(self, host="10.94.47.24", port=3611, user="nuomi", passwd="123456", db="dipper", charset="utf8"):
    ...
    def connectDb(self):
        connKey = "%s_%s_%s_%s_%s" % (self.host, self.port, self.user, self.passwd, self.db)
        if connKey not in self.connPool:
            self.reconnectDb(connKey)
        else:
            try:
                self.connPool[connKey].ping()
            except Exception, ex:
                self.reconnectDb(connKey)
        self.conn = self.connPool[connKey]
        self.cursor = self.conn.cursor()
        
    def reconnectDb(self, connKey):
        r = MySQLdb.connect(host=self.host, port=self.port, user=self.user, passwd=self.passwd, \
                db=self.db, charset=self.charset)
        self.connPool[connKey] = r
    ...
    def executeInsert(self, sql):
        try:
            ...
            comm = self.conn.commit()
        except Exception, ex:
            return {"errno":1, "insert_id":False, "errmsg":"%s"%(ex)}
        return {"errno":0, "insert_id":insert_id, "errmsg":""}

    def executeInsertMany(self, sql, param):
        """
        @breief: 批量插入数据,输入是sql和list
        @return: 
        """
        ...
        try:
            ...
            return {"errno":1, "data":False, "errmsg":"%s"%(ex)}
        return {"errno":0, "data":True, "errmsg":""}
    ...
    def __del__(self):
        self.cursor.close()
        for each in self.connPool :
            self.connPool[each].close()
